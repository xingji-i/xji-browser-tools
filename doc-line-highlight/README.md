...
- **位置记忆** - 关闭页面时保存当前行索引 + 文本内容 + URL，重新打开时通过文本匹配（优先）或索引回退（兜底）恢复位置
...
- 关闭页面时将 `当前行索引 + 当前行文本 + 页面 URL` 保存到 `chrome.storage.local`
- 重新打开同一页面时，优先通过文本内容匹配恢复位置（精确），文本匹配失败时回退到索引定位（近似）
